...
import { ArrowRight, Calendar, MapPin, Users, Award, ShieldAlert, Video, Image as ImageIcon } from "lucide-react";
          ...
          <div className="p-6 md:p-10 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-b border-white/5 pb-6 text-sm text-gray-400 font-mono">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-[10px] text-gray-500 block">التاريخ والموعد:</span>
                  <span className="text-white text-xs">{startFormatted} - {endFormatted}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-neon-cyan" />
                <div>
                  <span className="text-[10px] text-gray-500 block">الموقع الجغرافي:</span>
                  <span className="text-white text-xs truncate max-w-[200px] block">{event.location}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Users className="w-4 h-4 text-neon-cyan" />
                <div>
                  <span className="text-[10px] text-gray-500 block">عدد الحضور التقديري:</span>
                  <span className="text-white text-xs">{event.attendeeCount}+ مشارك مهتم</span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  {event.partners.map((partner: any) => (
                    <div key={partner.id} className="bg-white/5 border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-between text-center space-y-3 hover:border-white/10 transition-colors">
                      <img
                        src={partner.logo}
                        alt={partner.name}
                      ...
                      <div className="space-y-1">
                        ...
                        {partner.description && (
                          <span className="text-[10px] text-gray-400 block line-clamp-1">{partner.description}</span>
                        )}
                      </div>
                      {partner.website && (
                        <a
