...
export default async function UserCertificateDetailsPage({ params }: PageProps) {
    ...
    <div className="w-full min-h-screen bg-gray-950 py-12 px-4 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="certificate-card w-full max-w-4xl bg-gray-900 border-4 border-double border-white/10 rounded-3xl p-8 md:p-16 text-center space-y-8 relative shadow-2xl relative z-10">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-neon-cyan font-bold tracking-[6px] text-xs uppercase font-mono">ComixDev Academy</span>
          ...
          <p className="text-xs text-gray-500 font-mono tracking-wider">CERTIFICATE OF ATTENDANCE</p>
        </div>
        ...
        <div className="space-y-6 max-w-2xl mx-auto text-gray-300">
          <p className="text-sm md:text-base leading-relaxed text-gray-400">
            تُمنح هذه الشهادة المعتمدة تقديراً لحضور وإتمام ورشة العمل التقنية بنجاح:
          </p>
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-8 max-w-3xl mx-auto">
          ...
          <div className="text-right space-y-1">
            <span className="text-[10px] text-gray-500 font-semibold block uppercase font-mono">Issued by</span>
            <span className="text-sm font-bold text-white block">إدارة ComixDev</span>
            <span className="text-xs text-neon-purple font-semibold block">أكاديمية التدريب والذكاء الاصطناعي</span>
          ...
          <div className="flex items-center gap-4 bg-white/5 border border-white/10 p-3 rounded-2xl">
            ...
            />
            <div className="text-right space-y-1">
              <span className="text-[10px] text-gray-500 font-semibold block">رمز التحقق للشهادة</span>
              <span className="text-[10px] text-neon-cyan font-mono font-bold block">{certificate.certificateNumber}</span>
              <p className="text-[9px] text-gray-400 max-w-[150px] leading-snug">
                امسح الرمز ضوئياً للتحقق الفوري من صحة وصلاحية هذه الشهادة عبر موقعنا الرسمي.
              </p>
            </div>
          </div>
