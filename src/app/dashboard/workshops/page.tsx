...
import { Calendar, Clock, Award, CheckCircle2, XCircle, BookmarkCheck, Loader2 } from "lucide-react";
...
export default async function UserDashboardWorkshopsPage({ searchParams }: PageProps) {
      ...
      <div className="container mx-auto px-4 max-w-5xl space-y-8 text-right" dir="rtl">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-6">
          ...
          <div className="flex items-center gap-2 bg-neon-cyan/5 border border-neon-cyan/15 rounded-lg px-4 py-2 text-neon-cyan">
            ...
            <span className="font-mono font-bold text-lg">{session.user.points ?? 0}</span>
          </div>
        </div>
        ...
        {registrations.length > 0 ? (
                  ...
                  <div className="flex items-center gap-4 self-end md:self-center">
                    ...
                    <Link
                      ...
                      className={buttonVariants({
                        variant: "outline",
                        className: "border-white/10 hover:bg-white/10 text-gray-300 px-4 text-xs h-9",
                      })}
                    >
                      صفحة الورشة
                    </Link>
          ...
          <div className="text-center py-20 bg-gray-950/20 rounded-xl border border-white/5 max-w-md mx-auto space-y-4">
            ...
            <p className="text-gray-400 text-sm">لا توجد ورشات مسجلة تطابق معايير البحث والفلترة المحددة.</p>
            <Link
              href="/workshops"
              className={buttonVariants({
